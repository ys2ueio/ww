...
-- YSLEM HUB — Steal a Brainrot
-- Version améliorée et corrigée du script original
-- ===================================================================
...
local setUnwalkToggle, setAutoLeft, setAutoRight
...
local State = {
	...
	unwalkEnabled=false,
	-- Keybinds (Unknown = pas de touche assignée)
...
-- Cache prompts avec invalidation sur mouvement
...
local function findNearestPrompts()
	local now = tick()
	if _promptCache and (now - _promptCacheTime) < PROMPT_CACHE_TTL then
		return _promptCache
	end
...
-- Invalidation du cache prompt sur mouvement
...
-- Init data prompt avec fallback automatique
...
local function executeSteal(prompt, animalName)
	...
	-- Progress bar update (un seul Heartbeat partagé)
	local progConn
...
-- ===================================================================
-- COULEURS (Yslem violet)
-- ===================================================================
local C_BG=Color3.fromRGB(10,10,12)
local C_PANEL=Color3.fromRGB(18,18,22)
local C_ROW=Color3.fromRGB(24,24,30)
local C_ROW_HOV=Color3.fromRGB(32,32,40)
local C_BORDER=Color3.fromRGB(45,40,70)
local C_BORDER2=Color3.fromRGB(70,55,110)
local C_HEADER=Color3.fromRGB(14,14,18)
local C_ACCENT=Color3.fromRGB(220,200,255)
local C_ACCENT2=Color3.fromRGB(180,140,255)
local C_DIM=Color3.fromRGB(90,90,100)
local C_WHITE=Color3.fromRGB(255,255,255)
local C_ON_BG=Color3.fromRGB(60,40,100)
local C_OFF_BG=Color3.fromRGB(22,22,28)
local C_KEY_BG=Color3.fromRGB(0,0,0)
local C_YSLEM=Color3.fromRGB(130,80,255)
...
pctLbl.Size = UDim2.new(0.42,0,0,28); pctLbl.Position = UDim2.new(0,14,0,7)
pctLbl.BackgroundTransparency = 1; pctLbl.Text = "0%"
pctLbl.TextColor3 = C_WHITE; pctLbl.Font = Enum.Font.GothamBlack; pctLbl.TextSize = 24
pctLbl.TextXAlignment = Enum.TextXAlignment.Left; pctLbl.ZIndex = 101

progressRadLbl = Instance.new("TextLabel", stealProgressBar)
progressRadLbl.Size = UDim2.new(0.58,-14,0,28); progressRadLbl.Position = UDim2.new(0.42,0,0,7)
progressRadLbl.BackgroundTransparency = 1; progressRadLbl.Text = "Steal Radius: "..AutoSteal.Radius
progressRadLbl.TextColor3 = C_WHITE; progressRadLbl.Font = Enum.Font.GothamBlack; progressRadLbl.TextSize = 18
...
barTrack.Size = UDim2.new(1,-28,0,14); barTrack.Position = UDim2.new(0,14,0,40)
barTrack.BackgroundColor3 = Color3.fromRGB(28,28,28); barTrack.BorderSizePixel = 0; barTrack.ZIndex = 101
...
local mainStroke = Instance.new("UIStroke", main); mainStroke.Color = C_YSLEM; mainStroke.Thickness = 1.5; mainStroke.Transparency = 0.3
...
headerDiv.BackgroundColor3 = C_YSLEM; headerDiv.BorderSizePixel = 0; headerDiv.ZIndex = 6
...
miniBtn.Name = "YslemMini"; miniBtn.Size = UDim2.new(0,150,0,30)
...
pad.PaddingLeft=UDim.new(0,8); pad.PaddingRight=UDim.new(0,8); pad.PaddingTop=UDim.new(0,8); pad.PaddingBottom=UDim.new(0,10)
...
local function makeSectionLabel(text)
	...
	lbl.Text=text:upper(); lbl.TextColor3=C_YSLEM; lbl.Font=Enum.Font.GothamBlack; lbl.TextSize=10; lbl.TextXAlignment=Enum.TextXAlignment.Left
end
...
local function makeInputRow(label, default, onChange)
	...
	box.FocusLost:Connect(function()
		...
		if num then local f = math.floor(math.clamp(num,0,500)); box.Text = tostring(f); if onChange then onChange(tostring(f)) end; autoSaveConfig()
...
local function makeToggleRow(label, defaultOn, onToggle, keybindStateKey)
	...
	Instance.new("UICorner",row).CornerRadius=UDim.new(0,8)
	local lblWidth = keybindStateKey and 0.55 or 0.7
	...
	if keybindStateKey then
		...
		local kbs = Instance.new("UIStroke", kbBtn); kbs.Color = Color3.fromRGB(50,40,80); kbs.Thickness = 1
		local listening = false
		local lConn = nil
		local function stopListen(k)
			listening = false
			if lConn then lConn:Disconnect(); lConn=nil end
			kbs.Color = Color3.fromRGB(50,40,80); kbBtn.TextColor3 = Color3.fromRGB(200,200,210)
		...
		kbBtn.MouseButton1Click:Connect(function()
			...
			listening = true; kbBtn.Text = "..."; kbBtn.TextColor3 = C_WHITE
			kbs.Color = C_YSLEM
	...
	local pStroke = Instance.new("UIStroke", pill); pStroke.Color = defaultOn and C_YSLEM or C_BORDER; pStroke.Thickness = 1
...
setAutoBat = makeToggleRow("Auto Bat (Aimbot)", false, function(on) State.autoBatToggled=on; if MobileButtons.Buttons.autoBat then MobileButtons.Buttons.autoBat(on) end end, "keyAutoBat")

makeGap(10)
makeSectionLabel("Auto Steal")
local radiusBox = makeInputRow("Grab Radius", AutoSteal.Radius, function(v)
	local n = tonumber(v); if n and n>=5 and n<=300 then AutoSteal.Radius = math.floor(n); if progressRadLbl then progressRadLbl.Text = "Steal Radius: "..AutoSteal.Radius end; _promptCache = nil end
end)
local durationBox = makeInputRow("Steal Duration", AutoSteal.Duration, function(v)
	local n = tonumber(v); if n and n>=0.1 and n<=10 then AutoSteal.Duration = n end
end)
...
footerLbl.Text = "Yslem Hub · v1.0"; footerLbl.TextColor3 = Color3.fromRGB(50,50,60)
footerLbl.Font = Enum.Font.Gotham; footerLbl.TextSize = 10; footerLbl.TextXAlignment = Enum.TextXAlignment.Center

-- ===================================================================
-- UI HELPERS
-- ===================================================================
setSpeedToggleUI = function(carry) end
setLaggerToggleUI = function(lagger) end
...
local function createMobilePanel()
	...
	local BTN_W,BTN_H,CORNER,GAP = 58,58,14,12
	...
	local function makeBtn(name, text, defaultPos, saveKey, callback, isToggle)
		...
		cont.BackgroundColor3 = Color3.fromRGB(0,0,0); cont.BorderSizePixel = 0
		cont.Size = UDim2.new(0,BTN_W,0,BTN_H); cont.Position = defaultPos
		Instance.new("UICorner", cont).CornerRadius = UDim.new(0,CORNER)
		...
		btn.TextColor3 = C_WHITE; btn.TextSize = 12; btn.TextWrapped = true; btn.AutoButtonColor = false
		local active = false
		local function setActive(s)
			active = s
			if s then cont.BackgroundColor3 = C_YSLEM; btn.TextColor3 = C_WHITE
			else cont.BackgroundColor3 = Color3.fromRGB(0,0,0); btn.TextColor3 = C_WHITE end
		end
	...
	local autoBat = makeBtn("BtnAutoBat","BAT\nAIMBOT",UDim2.new(1,COL1_X,BASE_Y,ROW_Y[2]),"YslemBtn_autobat.txt",function(setA) State.autoBatToggled=not State.autoBatToggled; setAutoBat(State.autoBatToggled); setA(State.autoBatToggled); autoSaveConfig() end, true)
...
local function setupChar(char)
	...
	-- Cleanup medusa connections au respawn (FIX MEMORY LEAK)
	stopMedusaCounter()
...
-- ===================================================================
-- NO COLLIDE (event-driven, plus de Stepped loop)
-- ===================================================================
...
local function loadConfig()
	...
	if cfg.grabRadius then AutoSteal.Radius = cfg.grabRadius; radiusBox.Text = tostring(cfg.grabRadius); if progressRadLbl then progressRadLbl.Text = "Steal Radius: "..cfg.grabRadius end end
